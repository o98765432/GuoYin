$(function () {
    ...
    $("#FileUpload").bind("change", function () {
        ...
        $("#form1").ajaxSubmit({
            ...
            url: "../../Tools/ProductMultipleUpload.ashx",
            type: "post",
            dataType: "json",
            timeout: 60000
        });
    ...
    //取第一张图片载入预览图
    var loadBoxUrl = $(".imgItems1 ul img").eq(0).attr("src");
    if (loadBoxUrl != null) {
...
//上传图片成功的处理类
function showResponse1(data, textStatus) {
    if (data.msg == 1) {
        var str = "<li><img src=\"" + data.msbox + "\" onmouseover=\"ChangePreview('" + data.msbox + "');\" /><a onclick=\"dlstItems_Command(this,'" + data.msgid + "','" + data.msbox + "');\">删除</a><input name=\"hideFiles\" type=\"hidden\" value=\"" + data.msbox + "\"></li>";
        $(".imgItems1 ul").append(str);
        ChangePreview(data.msbox);
    } else {
        alert(data.msbox);
    }
...
function dlstItems_Command(obj, fileid, imgurl) {
    ...
    $.ajax({
        type: "post",
        url: "../../Tools/Delete_ProductAlbumFile.ashx",
        data: {
            fileid: function() {
                return fileid;
            },
...
//光标经过图片时显示预览图
function ChangePreview1(imgurl) {
    $(".imgbox1").empty();
    $(".imgbox1").css({ background: "none" });
    $(".imgbox1").append("<img src=\"" + imgurl + "\" />");
}